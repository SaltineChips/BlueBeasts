...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BlueBeasts-Coin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BlueBeasts-Coin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BlueBeasts-Coin is not connected!");

    //if (IsInitialBlockDownload())
    //    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BlueBeasts-Coin is downloading blocks...");
