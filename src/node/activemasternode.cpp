...
//
// Bootup the masternode, look for a 65,000 TBB input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in BlueBeasts-Coin.conf */
...
{
    CBlueBeastsCoinAddress address(collateralAddress);
