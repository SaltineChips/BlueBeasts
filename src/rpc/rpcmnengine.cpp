...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in BlueBeasts-Coin.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in BlueBeasts-Coin.conf\n"
                ...
                "  vote-many    - Vote on a BlueBeasts-Coin initiative\n"
                "  vote         - Vote on a BlueBeasts-Coin initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CBlueBeastsCoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "status")
    {
        ...
        if (pmn) mnObj.push_back(Pair("pubkey", CBlueBeastsCoinAddress(pmn->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a BlueBeasts-Coin initiative (can be additionally filtered, partial match)\n"
