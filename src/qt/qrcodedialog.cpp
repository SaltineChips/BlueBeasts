...
{
    QString ret = QString("BlueBeasts-Coin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TBB unit input in lnReqAmount, we generate the URI with TBB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BlueBeastsCoinUnits::format(BlueBeastsCoinUnits::TBB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
