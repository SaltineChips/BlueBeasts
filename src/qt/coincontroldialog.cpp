...
{
    GUIUtil::setClipboard(BlueBeastsCoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BlueBeastsCoinUnits::TBB;
    ...
    l2->setText(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)).arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)).arg(BlueBeastsCoinUnits::formatWithUnit(nDisplayUnit, CENT)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CBlueBeastsCoinAddress(outputAddress).ToString());
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BlueBeastsCoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BlueBeastsCoinUnits::format(nDisplayUnit, nSum));
