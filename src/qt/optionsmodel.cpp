...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BlueBeastsCoinUnits::TBB);
    ...
    if (!settings.contains("nAnonymizeBlueBeastsCoinAmount"))
        settings.setValue("nAnonymizeBlueBeastsCoinAmount", 1000);
    nAnonymizeBlueBeastsCoinAmount = settings.value("nAnonymizeBlueBeastsCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBlueBeastsCoinAmount"))
        SoftSetArg("-anonymizeBlueBeastsCoinamount", settings.value("nAnonymizeBlueBeastsCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBlueBeastsCoinAmount:
            return QVariant(nAnonymizeBlueBeastsCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeBlueBeastsCoinAmount:
            nAnonymizeBlueBeastsCoinAmount = value.toInt();
            settings.setValue("nAnonymizeBlueBeastsCoinAmount", nAnonymizeBlueBeastsCoinAmount);
            emit AnonymizeBlueBeastsCoinAmountChanged(nAnonymizeBlueBeastsCoinAmount);
            break;
        default:
            break;
        }
