...
{
    widget->setMaxLength(BlueBeastsCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new BlueBeastsCoinAddressValidator(parent));
...
bool parseBlueBeastsCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("BlueBeasts-Coin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BlueBeastsCoinUnits::parse(BlueBeastsCoinUnits::TBB, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBlueBeastsCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert bluebeastscoin:// to BlueBeastsCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bluebeastscoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "BlueBeastsCoin:");
    }
    QUrl uriInstance(uri);
    return parseBlueBeastsCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BlueBeasts-Coin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BlueBeasts-Coin.lnk
...
{
    return GetAutostartDir() / "BlueBeasts-Coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a BlueBeasts-Coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BlueBeasts-Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BlueBeasts-Coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BlueBeasts-Coin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BlueBeasts-Coin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BlueBeasts-Coin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BlueBeasts-Coin-Qt"));
